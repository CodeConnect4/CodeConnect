...
const DisplayFavourites = () => {
  ...
  useEffect(() => {
    ...
    fetchFavoritePostsFunction({
      ...
      if (data) {
        ...
        setPosts(data); // Set initial posts data
      } else {
        setIsError(true);
        setAlertMessage("Unable to connect to the server :(");
        setResponse(true);
  ...
  }, [fetchFavoritePostsFunction, fetchTagsFunction, userToken]);
  
  useEffect(() => {
    setPosts(prevPosts => {
      let filteredPosts = [...prevPosts]; // Create a copy of previous posts state
      ...
      if (searchField !== "") {
        filteredPosts = filteredPosts.filter(({ postData, ownerInfo }) =>
      ...
      switch (sortCriteria) {
        case "most_recent":
          filteredPosts.sort((a, b) => b.postData.createdAt - a.postData.createdAt);
          break;
        case "most_votes":
          filteredPosts.sort((a, b) => b.postData.upVotes.length - a.postData.upVotes.length);
          break;
        default:
          break;
      }
