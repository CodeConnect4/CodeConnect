...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    fetchAllPostsFunction().then(({ data, error }) => {
      if (data) {
        ...
        // Apply filtering on allPostsData and set the filtered posts
        let filteredPosts = [...data];
  
        if (selectedTags.length > 0) {
          filteredPosts = filteredPosts.filter(({ postData }) =>
        ...
        if (searchField !== "") {
          filteredPosts = filteredPosts.filter(
            ({ postData, ownerInfo }) =>
              ...
              ownerInfo?.name.toLowerCase().includes(searchField.toLowerCase()) ||
              postData?.description.toLowerCase().includes(searchField.toLowerCase()) ||
              postData?.tags?.some((tag) => tag?.toLowerCase().includes(searchField.toLowerCase()))
          );
        }
        ...
        switch (sortCriteria) {
          case "most_votes":
            filteredPosts.sort(
              (a, b) =>
                b.postData.upVotes.length + b.postData.downVotes.length -
  ...
  }, [fetchTagsFunction, fetchAllPostsFunction, selectedTags, searchField, sortCriteria]);
